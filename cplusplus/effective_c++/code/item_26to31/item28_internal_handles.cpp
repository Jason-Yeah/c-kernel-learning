...
{
public:
    explicit Playlist(std::vector<std::string> titles)
        : titles_(std::move(titles))
    {
    }

    const std::string &titleAt(std::size_t index) const
    {
        return titles_.at(index);
    }
    std::string titleCopyAt(std::size_t index) const
    {
        return titles_.at(index);
    }
...
int main()
{
    ...
    // std::vector 的元素通常连续存放在一块堆内存中，而 vector
    // 容量不够时，必须换一块更大的内存
