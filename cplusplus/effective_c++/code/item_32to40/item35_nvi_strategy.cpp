...
public:
    ...
    int health() const { // NVI：固定入口统一校验。
        const int value = doHealth();
        if (value < 0) throw std::logic_error("negative health");
        return value;
    }
private:
...
private:
    int doHealth() const override { return value_; } // 可以覆盖基类 private 虚函数。
    int value_;
};
...
public:
    ...
    StrategyCharacter(int base, Algorithm algorithm)
        ...
        if (!algorithm_) throw std::invalid_argument("empty strategy");
    }
    int health() const {
...
int main() {
    ...
    const int damage = 25;
    StrategyCharacter injured{100, [damage](int base) { return base - damage; }};
