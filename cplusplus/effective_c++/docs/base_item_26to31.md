...
这样每轮只为有效请求构造 `record`，逻辑清楚。旧书会讨论“循环外构造一次、循环内赋值”与“每轮构造析构”的成本取舍；现代 C++ 中优先保证**语义正确**和**最小作用域**，再用性能分析决定是否值得复用对象。复用对象还可能保留容量、状态或异常后的部分修改，复杂度更高。
...
`dynamic_cast` 的实现通常依赖 RTTI（Runing-Time Type Information 运行时类型信息），对象的虚函数表/类型信息可帮助判断真实派生类型；具体布局属于 ABI 实现细节。`static_cast` 不做这类检查，因此通常更轻，但“轻”不等于可以猜测类型。
...
> `std::string_view`字符串轻量视图，只记住地址+长度，不拷贝不申请新的内存地址
> `std::span`一段连续对象的轻量视图，某一段连续内存里的 T 对象，只存地址不拷贝
...
多个 `.cpp` 包含这个头文件时，每个翻译单元都会看见函数定义；因 `inline`，只要定义一致，这是合法的。

> 类定义内部定义的成员函数、`constexpr` 函数也隐含 inline 语义。
