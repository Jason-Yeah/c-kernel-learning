...
### 9.4 buffer——ASIO 的数据缓冲区设计（核心）

这是 ASIO 里**设计最精妙但也最容易用错**的概念。请仔细看。

#### 9.4.1 核心问题：网络 I/O 需要什么样的数据缓冲区？

回想 C 语言里你怎么读写 socket：

```c
char buf[1024];
int n = recv(fd, buf, sizeof(buf), 0);  // 把数据读到 buf 里
send(fd, buf, n, 0);                     // 把 buf 里的数据发出去
```

这里 `buf` 是一个**原始的内存地址**（`char*`），`recv` 和 `send` 就认两样东西：**起始地址 + 长度**。

那 ASIO 为什么不用同样的方式？比如：

```cpp
// 如果 ASIO 也这么设计（实际上不是）
char buf[1024];
socket.async_read_some(buf, 1024, callback);  // 为什么不这样？
```

原因有三个：

1. **类型不安全**——`char*` 没有携带长度信息，你不知道缓冲区有多大
2. **读写不分**——`send` 用 `const char*`（不修改），`recv` 用 `char*`（要修改），C 语言靠两个不同的系统调用区分，ASIO 的异步接口想更明确
3. **多样性**——缓冲区可能来自栈数组、`std::vector`、`std::string`、`malloc` 出来的堆内存，ASIO 希望**统一处理它们**

**ASIO 的解决方案**：引入 `asio::buffer()` 包装函数，把"内存地址 + 长度 + 读写属性"打包成一个统一的对象。

#### 9.4.2 buffer 的核心理念——"视图"而非"容器"

这是最关键的一句话：

> **`asio::buffer()` 不分配内存、不拷贝数据，它只是记录下"你的数据在哪里、有多长"。**

它本质上就是这两个东西的打包：

```cpp
// asio::buffer 内部大致是这样（伪代码）
struct buffer {
    void* data;      // 内存起始地址
    size_t size;     // 数据长度
    bool writable;   // 是否可写
...
**你不是把数据交给 ASIO，你是把数据的地址告诉 ASIO。** ASIO 不会把你的 `char[1024]` 拷贝到自己内部——它只记下指针。

**为什么设计成"视图"而不是"拷贝"？**

因为网络 I/O 本身就是"把数据从 A 搬到 B"的零和游戏。如果 ASIO 每次再从你的缓冲区拷贝一份到自己内部，就是**双重拷贝**：

```
你的数据 → ASIO 内部缓冲区（第 1 次拷贝）→ 内核缓冲区（第 2 次拷贝）→ 网卡
```

ASIO 的设计消除了第一次拷贝：

```
你的数据 → 内核缓冲区（唯一一次拷贝）→ 网卡
          ↑ ASIO 直接告诉内核"数据在你的缓冲区里，从这里读"
```

这就是 ASIO buffer **零拷贝（zero-copy）** 的设计理念——它只是内存视图，不做中间人。

#### 9.4.3 mutable_buffer vs const_buffer——读写分离

ASIO 内部有两种 buffer 类型：

| 类型 | 语义 | 用于 |
|------|------|------|
| `mutable_buffer` | 可读可写的内存 | 读操作（`async_read_some`、`async_read`） |
| `const_buffer` | 只读的内存 | 写操作（`async_write`、`async_write_some`） |

你**不会直接操作这两个类型**——你用的是 `asio::buffer()` 函数，它会**自动推断**：

```cpp
char data[1024];
asio::buffer(data)          → mutable_buffer  （数组可读写）
asio::buffer(data, 512)     → mutable_buffer  （指定长度）

const char data2[] = "hello";
asio::buffer(data2)         → const_buffer    （const 数组只读）

std::string str = "world";
asio::buffer(str)           → const_buffer    （string 的内容只读）

std::vector<char> vec(1024);
asio::buffer(vec)           → mutable_buffer  （vector 内容可读写）
```

**关键设计**：C 语言靠程序员记住"这个 buffer 是给 recv 还是 send 用的"，ASIO 靠**类型系统**确保你不会把只读数据传给写操作。

#### 9.4.4 buffer 的多种构造方式

`asio::buffer()` 的**重载**（同一个函数名，不同的参数类型）可以接受几乎所有常见的数据源：

| 传入参数 | 效果 | 用途 |
|---------|------|------|
| `asio::buffer(arr)` | 自动推断数组长度 | `char buf[1024]` |
| `asio::buffer(arr, n)` | 指定长度 | 只想用数组的前 n 个字节 |
| `asio::buffer(vec)` | 用 `std::vector` 的全部数据 | 动态增长的缓冲区 |
| `asio::buffer(str)` | 用 `std::string` 的内容（只读） | 直接发字符串 |
| `asio::buffer(str, n)` | string 的前 n 个字符 | 发字符串的一部分 |
| `asio::buffer(p, n)` | 原始指针 + 长度 | 自己管理的内存 |
...
// asio::buffer(buf) 自动得到大小 1024

// 2. std::vector
std::vector<char> vec(1024);
socket.async_read_some(asio::buffer(vec), callback);

// 3. std::string（只读，用于发送）
std::string msg = "Hello";
asio::async_write(socket, asio::buffer(msg), callback);
```

**不用 `asio::buffer()` 手动传指针+长度行不行？**

```cpp
// 错误示范：别这么写
socket.async_read_some(buf, 1024, callback);  // 编译错误！
```

不行——ASIO 的异步接口要求参数是 buffer 类型，不能是裸指针。

#### 9.4.5 最关键的陷阱：buffer 的生命周期

> **传给 `async_read` / `async_write` 的 buffer 在回调执行之前必须保持有效。**

这是 ASIO 初学者**最常犯的错误**，一定要理解为什么。

**错误代码 #1——局部变量被销毁：**

```cpp
void bad_case() {
    char data[1024];                          // data 是栈上局部变量
    socket.async_read_some(asio::buffer(data), [](ec, n) {
        // 这里的回调可能在 5 秒后才执行
        // 但 data 在 bad_case() 返回时就已经被销毁了！
    });
    // bad_case 返回 → data 的栈内存被回收
    // 5 秒后 ASIO 往 data 地址写数据 → 写入的是"别人的栈" →
    //    要么崩溃，要么数据错误，要么写出安全漏洞
}
```

**正确做法**：把 buffer 的生命周期延长到回调执行之后。

```cpp
// 方案 A：使用智能指针管理缓冲区
auto data = std::make_shared<std::array<char, 1024>>();
socket.async_read_some(asio::buffer(*data),
    [data](ec, n) {  // 捕获 shared_ptr，data 不会提前销毁
        // 回调执行完后，data 才会被释放
    });

// 方案 B：buffer 是成员变量（在 Session 类的 data_ 数组）
// 第 10 节的 echo server 就这么做的——data_ 是 Session 的成员
// Session 对象（shared_ptr）活着，data_ 就活着
```

**为什么 ASIO 不帮你拷贝一份？**

因为**零拷贝设计决定了 buffer 是视图而非容器**。如果 ASIO 每次读到数据都在内部拷贝一份，就不存在生命周期问题——但性能成本是巨大的（每个字节多拷贝一次，还要做堆分配）。ASIO 把生命周期管理的责任交给了你，换来了性能。

**总结成原则**：

```
buffer 是"借"给 ASIO 用的内存，不是"给"ASIO 的内存。
借的东西不能提前收回。
```

#### 9.4.6 streambuf——当你不知道数据有多大的时候

上面的 buffer 都需要你**预先知道**缓冲区大小（`char data[1024]`）。但如果不知道呢？比如要读一个 HTTP 请求，头部的长度是变化的。

ASIO 提供了 `asio::streambuf`（继承自 `std::streambuf`），可以**自动增长**：

```cpp
asio::streambuf buf;                    // 开始时是空的
asio::async_read_until(socket, buf, "\r\n\r\n",  // 读到 HTTP 头部结束标记
    [&buf](ec, n) {
        // 数据已经读进 buf 了
        std::istream is(&buf);          // 用输入流访问数据
        std::string line;
        std::getline(is, line);         // 读一行
    });
```

`streambuf` 的核心特性：

- **自动扩容**——数据超过当前容量时自动分配更多内存
- **输入输出流接口**——可以用 `std::istream` / `std::ostream` 读写
- **配套函数**——`async_read_until`（读到某个分隔符为止）专门配合 streambuf 使用

**streambuf 的代价**：因为要自动扩容，它在内部会做**堆分配和拷贝**（当现有空间不够时）。所以它不是零拷贝的——它在内部拥有数据的所有权。

#### 9.4.7 Buffer Sequence——一次读写多个缓冲区（Scatter-Gather）

这是 ASIO 另一个强大的设计——**buffer 序列**。你可以一次给 ASIO **多个** buffer：

```cpp
// 发送时：把三个不连续的内存块一次性发出去
std::string header = "HTTP/1.1 200 OK\r\n";
std::vector<char> body = get_body();  // 假设这个函数返回 body
std::string footer = "\r\n";

std::vector<asio::const_buffer> buffers;
buffers.push_back(asio::buffer(header));
buffers.push_back(asio::buffer(body));
buffers.push_back(asio::buffer(footer));

asio::async_write(socket, buffers, callback);
// ASIO 会把三个 buffer 连续发出去，就像发了一段连续的字符串
```
...
send(fd, footer, footer_len, 0);
// 三次系统调用，三次用户态↔内核态切换
```

ASIO 的 buffer sequence：

```
一次 async_write + 三个 buffer
    │
    │  ASIO 内部要么：
    │  方案 A：调用 writev(fd, iov, 3)  → 内核帮你"收集"多个缓冲区一次发
    │        （writev 是 Linux 的"集中写"系统调用，一次传多个缓冲区）
    │  方案 B：先把 buffer 拼成连续内存再 write
    │
    ▼
一次系统调用完成，省了两次上下文切换
```
...
```cpp
// 读操作也可以传多个 buffer
std::array<char, 4> header_buf;   // 4 字节头部
std::vector<char> body_buf(1024); // body 缓冲区

std::vector<asio::mutable_buffer> read_bufs;
read_bufs.push_back(asio::buffer(header_buf));
read_bufs.push_back(asio::buffer(body_buf));

socket.async_read_some(read_bufs, callback);
// ASIO 先填满 header_buf，再填 body_buf
// 对应 Linux 的 readv() 系统调用（"分散读"）
```

**名称**：读多个 buffer 叫 **scatter（分散）**，写多个 buffer 叫 **gather（收集）**，合称 **scatter-gather I/O**。这是 ASIO 性能设计的一部分——减少系统调用次数。

#### 9.4.8 常见 buffer 陷阱速查

| 问题 | 错误写法 | 正确做法 |
|------|---------|---------|
| 局部 buffer 被销毁 | `char buf[1024]; async_read(..., buf, ...)` | 用类成员或 shared_ptr 管理生命周期 |
| 重复使用 buffer | 两次 `async_read` 共用同一个 buffer，第一次的回调还没执行 | 每次读分配新的 buffer，或用队列管理 |
| 混淆读写属性 | 把 `const char*` 传给 `async_read` | `const` → 只能用于 `async_write` |
| string 被修改 | `std::string s; async_write(..., s, ...)` 然后立即 `s.clear()` | buffer 是视图，回调完成前不要修改原始数据 |
| 忘记 ASIO 不拷贝 | 认为 ASIO 已经"拿走"了数据，提前释放 | 记住 buffer 只是"借"给 ASIO 的视图 |

#### 9.4.9 buffer 设计理念总结（一句话）

> **ASIO 的 buffer 是一条"便签"——它不保管你的数据，只告诉内核"数据在这"。零拷贝是它追求的目标，生命周期是你需要付出的代价。**
