...
工作中会利用auto和decltype配合使用，结合模板做类型推导返回动态类型

### decltype & auto

decltype只推断类型不做计算（静态分析），会原封不动地保留所有 const 和引用
...
auto 在推导类型时，遵循的是“值拷贝”的逻辑（类似模板参数推导）

- 忽略顶层const：拷贝的是一个常量，拷贝出来的新变量本身不需要是常量
- 保留底层const：拷贝一个指针，必须保证不能通过新指针去修改原始数据。
